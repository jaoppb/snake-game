...
export default class Game extends ScreenBase {
    ...
    constructor() {
        ...
        const HUD = this.elements.createChild("HUD", document.createElement("div"), true);
        const back = HUD.createChild("back", document.createElement("div"), true);
        for (let i = 0; i < 3; i++)
            back.createChild(`span-${i}`, document.createElement("span"));
        back.element.addEventListener("click", () => screenManager.setScreen(menu));
        this.score = {
            value: 0,
            element: HUD.createChild("score", document.createElement("span"))
        };
        const canvas = this.elements.createChild("canvas", document.createElement("canvas"));
    ...
    requestDirectionChange(newDir) {
        if (!newDir.map(v => -v).every((v, i) => v !== this.direction[i]))
            return;
    ...
    setSize() {
        ...
        if (Object.keys(newSize).every(key => size[key] != undefined)
            ...
            && newSize.pixel == size.pixel)
            return;
        const { player, canvas } = this;
        const { element } = canvas;
        for (const key of Object.keys(newSize))
        ...
        player.tail = [player.head.map((value, index) => value - this.direction[index])];
        this.generateNewFruit();
    }
    generateNewFruit() {
        do
            this.fruit = this.getRandomVector2(this.size.pixel);
    ...
    update() {
        ...
        player.head = player.head.map((v, i) => {
            ...
            if (value > this.size.pixel - 1)
                value %= this.size.pixel;
            else if (value < 0)
                value += this.size.pixel;
            return value;
        });
        ...
        if (player.head.every((v, i) => v == this.fruit[i])) {
            ...
            player.tail.pop();
        if (player.tail.some(v => v.every((v, i) => v == player.head[i])))
            this.running = false;
    }
    render() {
        ...
        if (toRender.some(value => value == undefined))
            return;
